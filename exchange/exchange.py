...
app = Flask(__name__) # создаем приложение дл прослушивания команд
...
def low_balance(userId, lotId, lowOn): # уменьшить баланс какого-то лота
    ...
    send_to_server(f"INSERT INTO user_lot VALUES ('{userId}', '{lotId}', '{quantity}')")
    return True
...
def create_user(): # создание пользователя
    ...
    if len(takenUsernames) != 0: # если пользователь с такимм именем уже существует
    ...
    send_to_server(f"INSERT INTO user VALUES ('{username}', '{key}')") # вставка пользователяя в бд
...
def change_status(orderId, userId, pairId, quantity, price, type, closeKey): # изменить состояние ордера с закрытого
    ...
    send_to_server(f"INSERT INTO order VALUES ('{userId}', '{pairId}', '{quantity}', '{price}', '{type}', '{closeKey}')")
...
def what_to_order(data, userId, firstLot, secondLot, oppositeType): # обработка покупки и продажи
    ...
    temp = send_to_server(f"SELECT order.order_id, order.user_id, order.quantity, order.price FROM order WHERE order.pair_id = '{data['pair_id']}' AND order.type = '{oppositeType}' AND order.closed = '-'").split('\n')[1:-2]

    # если нет таких ордеров, то просто вставляем в таблицу ордер и отправляем ID пользователю
    if (len(temp) == 0):
        send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{data['quantity']}', '{data['price']}', '{data['type']}', '-')")
    ...
    # выыбирааем ордеры с подходящей ценой
    if data['type'] == 'buy':
        orderMatrix = [[j for j in i.split(';')[:-1]] for i in temp if float(i.split(';')[3]) <= data['price']]
    else:
        orderMatrix = [[j for j in i.split(';')[:-1]] for i in temp if float(i.split(';')[3]) >= data['price']]
    ...
    for order in orderMatrix: # проходим по всем найденным запросам
        ...
        if data['quantity'] > float(order[2]):
            if data['type'] == 'buy':
                ...
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{order[2]}', '{order[3]}', '{data['type']}', '{closeKey}')")
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{data['quantity']}', '{data['price']}', '{data['type']}', '-')")
                ...
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{order[2]}', '{data['price']}', '{data['type']}', '{closeKey}')")
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{data['quantity']}', '{data['price']}', '{data['type']}', '-')")
        ...
        # если найденный запрос полностью покрывает текущий
            ...
            if data['type'] == 'buy':
                ...
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{data['quantity']}', '{order[3]}', '{data['type']}', '{closeKey}')")

                with open('../bin/trader/order/order_pk_sequence.txt', 'r') as pkFile:
                    returnId = int(pkFile.readline().strip()) - 1

                change_status(order[0], order[1], data['pair_id'], data['quantity'], order[3], f'{oppositeType}', closeKey)
                send_to_server(f"INSERT INTO order VALUES ('{order[1]}', '{data['pair_id']}', '{order[2] - data['quantity']}', '{order[3]}', '{oppositeType}', '-')")
                ...
                send_to_server(f"INSERT INTO order VALUES ('{userId}', '{data['pair_id']}', '{data['quantity']}', '{data['price']}', '{data['type']}', '{closeKey}')")
        ...
        # если значения совпали (то есть не надо создавать доп. запрос)
            ...
            print(f"Count {data['quantity']} my price {data['price']} his price {order[3]}")
...
def create_order():
    ...
    if len(keyCheck) == 0: # проверка наличия пользователя в системе
    ...
    returnId = str(what_to_order(data, userId, firstLot, secondLot, oppositeType))
    if returnId.isdigit():
...
if __name__ == '__main__':
    ...
    app.run(debug=True) # начинаем прослушивать консоль
