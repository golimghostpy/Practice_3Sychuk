...
/*
    ...
    for (auto i : lots["lots"])
    {
        string fillRequest = format("INSERT INTO lot VALUES ('{}')", (string)i);
        insert_into(name, split(fillRequest, " "));
    }

    StringMatrix temp;
    for (auto i = 0; i < lots["lots"].size() - 1; ++i)
    {
        temp = select_from(name, split(format("SELECT lot.id FROM lot WHERE lot.name = '{}'", (string)lots["lots"][i]), " "));
        string firstLotId = temp.get_at(0, 1)->data;
        for (auto j = i + 1; j < lots["lots"].size(); ++j)
        {
            temp = select_from(name, split(format("SELECT lot.id FROM lot WHERE lot.name = '{}'", (string)lots["lots"][j]), " "));
            string secondLotId = temp.get_at(0, 1)->data;
            insert_into(name, split(format("INSERT INTO pair VALUES ('{}', '{}')", firstLotId, secondLotId), " "));
        }
    }
    temp.clear();
